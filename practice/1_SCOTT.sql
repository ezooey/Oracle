...
-- 2. EMP 테이블에서 커미션을 받지 못 하는 직원 조회
SELECT *
FROM EMP
...
-- 6. EMP 테이블에서 사원번호, 사원명, 직급, 입사일 조회 (단, 입사일 기준 오름차순 정렬)
...
-- 12. EMP 테이블에서 사원명, 입사일 조회 (단, 입사일은 연도와 월을 분리 추출해서 출력)
SELECT ENAME, EXTRACT(YEAR FROM HIREDATE) HIRE_YEAR, EXTRACT(MONTH FROM HIREDATE) HIRE_MONTH
FROM EMP;
...
SELECT EMPNO, ENAME, HIREDATE, ADD_MONTHS(HIREDATE, 480) "입사일로부터 40년 후"
FROM EMP;
